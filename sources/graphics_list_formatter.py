...
DAY_TIME_EMOJI = [
    ...
    "🌃",
    "🌙",
]  # Emojis, representing different times of day.
DAY_TIME_NAMES = [
    "Morning",
    "Daytime",
    "Evening",
    "Night",
]  # Localization identifiers for different times of day.
WEEK_DAY_NAMES = [
    "Monday",
    "Tuesday",
    "Wednesday",
    ...
    "Saturday",
    "Sunday",
]  # Localization identifiers for different days of week.
...
async def make_commit_day_time_list(
    ...
    for repository in repositories:
        ...
        # Deduplicate commits across branches using commit SHA
        unique_commits = {}
        for branch in commit_dates[repository["name"]].values():
            for sha, committed_date in branch.items():
                if sha not in unique_commits:
                    unique_commits[sha] = committed_date

        for committed_date in unique_commits.values():
